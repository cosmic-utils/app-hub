...
pub fn app_image_update(
    ...
    info!("Updating app images from {} to {}", old_install_dir, new_install_dir);
    ...
    for desktop_entry in desktop_entries {
        let mut desktop_file_builder = match DesktopFileBuilder::from_desktop_entry_path(&PathBuf::from(&desktop_entry), true) {
            Ok(builder) => builder,
            Err(e) => {
        ...
        if desktop_file_builder.exec().is_none() {
            info!("Failed to parse desktop entry (exec missing): {}", &desktop_entry);
            continue;
        }

        let new_exec = desktop_file_builder.exec().unwrap().replace(&old_install_dir, &new_install_dir);
        info!("New exec: {}", new_exec);
        desktop_file_builder.set_exec(new_exec);

        // Update path fields
        if desktop_file_builder.path().is_some() {
            desktop_file_builder.set_path(desktop_file_builder.path().unwrap().replace(old_install_dir.as_str(), new_install_dir.as_str()));
        }
    ...
    if let Err(e) = Command::new("chmod")
        ...
        .stderr(Stdio::piped())
        .spawn() {
        return Err(format!("Failed to add executable permissions to app images: {}", e));
    }
